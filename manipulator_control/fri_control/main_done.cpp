...
{   
    ...
    UDPServer<7,14> server("127.0.0.1", 8082, "127.0.0.1", 8080);
    ...
    const double e = 0.2*M_PI/180;      // Доверительный интревал, попав в который, мы считаем, что пришли в точку
    ...
    while (true)
    {
        if (server.getMsg(msg_thetta))  // Чтение пришедших по UDP данных. Если сообщение есть getMsg возвращает true и перезаписывает переменную msg_thetta
        {
            ...
            next_point = msg_thetta;
            // points_queue.push(msg_thetta);
        };      

        // ========================================================================================

        current_point = stdArrayToEigenArray(kuka.getMeasuredJointPosition()); // Не забыть про преобразование типов

        std::cout << current_point.transpose()*180/M_PI << std::endl;

        if (false)   // Если манипулятор пришел в целевую точку
        {
        ...
        {
            dq = getDelta(next_point, temp); // Реализовать эту функцию в func.hpp
            temp = temp + dq;
            
            kuka.setTargetJointPosition(eigenArrayToStdArray(temp));

            if (eigenArrayEqual(temp,next_point,eps))  // Реализовать эту функцию в func.hpp
            {
                done = true;
